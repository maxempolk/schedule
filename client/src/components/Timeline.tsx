...
const Timeline: FC<TimelineProps> = ({ viewDay, todayJs, now, nowMin }) => {
  ...
  return (
    <div className="tl-rel" style={{ height: totalH * PX }}>
      ...
      {evts.map((e, i) => {
        ...
        return (
          <div
            ...
            style={{
              top: toTop(e.h, e.m),
              height: Math.max(toH(e.dur) - 3, 34),
              background: col.bg,
              borderLeftColor: col.border,
            }}
          >
            <div className="evt-name" style={{ color: col.text }}>
              {e.label}
            </div>
            <div className="evt-time" style={{ color: col.text }}>
              {fmt(e.h, e.m, e.dur)}
            </div>
          </div>
        );
      ...
      {showNeedle && <div className="needle" style={{ top: needleTop }} />}
    </div>
  );
};
